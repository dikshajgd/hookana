/**
 * Seeds the Sanity `portfolioPage` singleton with the current portfolio media.
 *
 * Usage:
 ...
 * After seeding, drag-and-drop the items in Sanity Studio
 * (/studio > Portfolio Page > Work / Media) to control the on-page order.
 */
...
// AI-generated reels. Japanese-themed video intentionally last.
const AI_URLS = [
...
let n = 0
const key = () => `item_${(++n).toString().padStart(4, "0")}`
const video = (url) => ({ _key: key(), _type: "portfolioVideo", url })
const image = (url) => ({ _key: key(), _type: "portfolioImage", url })
const mixedVideo = (url) => ({ _key: key(), _type: "portfolioMixed", kind: "video", url })
const mixedImage = (url) => ({ _key: key(), _type: "portfolioMixed", kind: "image", url })

const aiItems = AI_URLS.map(video)
const staticItems = IMAGE_URLS.map(image)
const videoItems = VIDEO_URLS.map(video)
// Default "All work" order matches the bundled fallback: AI → Static → Videos.
const allWorkItems = [
  ...AI_URLS.map(mixedVideo),
  ...IMAGE_URLS.map(mixedImage),
  ...VIDEO_URLS.map(mixedVideo),
]
...
console.log(
  `Seeded "${result._id}" in dataset "${dataset}": ` +
    `${aiItems.length} AI, ${staticItems.length} Static, ${videoItems.length} Videos, ` +
    `${allWorkItems.length} All work.`
)
console.log(
  "Open /studio > Portfolio Page. Each tab has its own list — drag within a list to reorder that tab independently."
)
